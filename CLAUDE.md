...
- **Time displacement / clone architecture.** Each island can be assigned a
  ...
  `N_SLOTS - 1` hidden `<video>` clones (`makeCloneManager`), each seeked to
  `(i / N_SLOTS) * duration` before playing; slot 0 always aliases the main
  `texA`/`texB` (no clone needed). `N_SLOTS` is derived from
  `MAX_TEXTURE_IMAGE_UNITS` at startup, clamped to `[1, 8]`.
