(() => {
  ...
  const N_SLOTS = Math.max(1, Math.min(8, Math.floor((maxTexUnits - 1) / 2)));
    ...
    function create() {
      ...
      for (let i = 1; i < N_SLOTS; i++) {
        ...
        clone.addEventListener('loadedmetadata', () => {
          ...
          if (dur > 0) {
            try { clone.currentTime = (i / N_SLOTS) * dur; } catch (e) {}
          }
  ...
  function updateIslandTexture() {
    ...
    const n = Math.max(1, Math.min(N_SLOTS, Number(timeGroupsSlider.value)));
  ...
  function refreshClonesForCurrentMask() {
    ...
    if (timeDisplacementEnabled && state.videoA && !aIsImage) cloneManagerA.create();
    else cloneManagerA.destroy();
    if (timeDisplacementEnabled && state.videoB && !bIsImage) cloneManagerB.create();
