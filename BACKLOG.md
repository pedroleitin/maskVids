...
- [ ] **Verify the Safari time-displacement fix on a real Safari browser.**
  The clone-count fix and the `N_SLOTS` cap (4 on Safari vs. 8 elsewhere) were
  built and reasoned through but not measured on actual Safari — the preview
  tooling here is Chromium-only. Needs a real before/after check on macOS/iOS
  Safari, and the cap of 4 may need tuning up or down once measured.
